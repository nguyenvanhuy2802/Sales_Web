    ...
    $(".notification-icon").on("click", function () {
        ...
        orders.forEach(order => {
            ...
            if (order.isVerified === 1) {
                ...
                actionButton = `<button class="btn btn-danger btn-sm" style="background-color: #b30000; color: white; border-color: #990000;" onclick="cancelOrder(${order.orderId})">Hủy đơn hàng</button>`;
            } else {
            ...
            const listItem = `
                <li class="list-group-item d-flex justify-content-between align-items-center ${badgeClass}" style="border-radius: 10px; margin-bottom: 10px;">
                    <span class="d-flex align-items-center gap-2">
                                    ${icon}
                        <span>Đơn hàng: ${order.orderId} ${verifyStatus}</span>
                    </span>
                    ${actionButton}
                </li>`;
